...
/* void initDAC1(void) {  /// init DAC1 on PA4 -> DAC1_OUT */
...
void initDmaDAC1(uint16_t* dBuff1, uint16_t* dBuff2, uint16_t size) {
  ...
  TIM6->PSC = 0x0054 - 0x0001;         // 1MHz
                                       //
  TIM6->ARR = 10;                      // 10 * 1/1MHz = 1e-5
  TIM6->CR1 = 0x0084;                  //
                                       //
  ...
  DMA1_Stream5->M0AR = (uint32_t)dBuff1;        //
  DMA1_Stream5->M1AR = (uint32_t)dBuff2;        //
